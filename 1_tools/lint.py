...
def find_orphans(pages: list[Path]) -> list[Path]:
    ...
    orphans = []
    for p in pages:
        if p == WIKI_DIR / "overview.md":
            continue
        # Papers: check they have outbound links, not inbound
        if "sources/papers" in str(p):
            content = read_file(p)
            has_outbound = bool(extract_wikilinks(content))
            if not has_outbound:
                orphans.append(p)
        # Notes, concepts, entities: original inbound check
        else:
            if inbound[p] == 0:
                orphans.append(p)
    return orphans
...
# Add this new function alongside it
def find_missing_papers(pages: list[Path]) -> list[str]:
    """
    Find [[wikilinks]] that look like paper references
    (cited in Related Notes sections) but have no page.
    Heuristic: link appears inside a ## Related Notes section.
    """
    existing_pages = {p.stem.lower() for p in pages}
    ...
    for p in pages:
        content = read_file(p)
        # Only look inside Related Notes sections
        match = re.search(
            r'##\s+Related Notes.*?(?=\n##|\Z)', content, re.DOTALL
        )
        if not match:
            continue
        section = match.group()
        for link in extract_wikilinks(section):
            if link.lower() not in existing_pages:
                missing[link] += 1

    return [name for name, count in missing.items() if count >= 1]
...
def run_lint():
    ...
    missing_papers = find_missing_papers(pages)

    print(f"  orphans: {len(orphans)}")
    print(f"  broken links: {len(broken)}")
    print(f"  missing entity pages: {len(missing_entities)}")
    print(f"  missing paper pages: {len(missing_papers)}")
    ...
    if missing_papers:
        report_lines.append("### Referenced Papers Not Yet Ingested")
        report_lines.append("> These papers are linked from Related Notes but have no wiki page.")
        report_lines.append("> Action: ingest them with `python tools/ingest.py raw/<paper>.md`")
